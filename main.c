...
static uint8_t patterns[] = {0b10101010,
	    0b01010101,
	    0b11001100,
	    0b00110011,
	    0b11110000,
...
static uint8_t errorValue = 0b00000001;
...
int main(void)
{
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  ...
	  if((GPIOA -> IDR & GPIO_IDR_0) == 0)//This checks if  the PAO button is pressed
	  	{
	  		uint16_t val = TIM16 -> ARR; // Get the current ARR value
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t nextPattern = read_from_address(currentAddress); // read next value from SPI

		if (!nextPattern == patterns[currentAddress])
		{
			GPIOB -> ODR = errorValue; //Indicate SPI value /write errorValue

		} else{
			GPIOB -> ODR = nextPattern;//load next pattern

		}
		//GPIOB -> ODR = 0b11110000;
		currentAddress = (currentAddress + 1)%6; // Move to next pattern
